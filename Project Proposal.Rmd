...
# first create plots for to check conditional mean predictor and conditional mean response conditions 

# pairs(clean_data[,3:15])
predictors <- clean_data[, c("TOTSTAFF","TOTPRO","TOTATTEN",
                             "STAFFEXP","TOTEXPCO","HRS_OPEN")]

# compute correlation matrix
cor_mat <- cor(predictors, use = "pairwise.complete.obs")  # handles missing values

# the correlation matrix
round(cor_mat, 2) 

par(mfrow=c(3,2))
important_vars <- c("TOTSTAFF","STAFFEXP","TOTATTEN","TOTEXPCO")
pairs(clean_data[important_vars])


plot(x = x_value, y = y_value2, main = "Response vs. Fitted", xlab = "Fitted", ylab = "Response - Visits")

# check homoscedasticity,linearity and normal errors assumptions with plot of residual vs fitted from initial model
plot (x = x_value, y = y_value, main="Residual vs. Fitted", xlab = "Fitted", ylab = "Residuals")

# check assumptions with categorical variable - location
...
tidy_mod <- broom::tidy(initial_model)
